...
def load_and_prepare_dataset():
    ...
    logging.info(f"Filtered dataset from {initial_size} to {len(full_dataset)} samples.")
    ...
    logging.info(f"Split dataset into {len(train_dataset)} training samples and {len(split_dataset['test'])} test samples.")
    ...
    if config.MAX_TRAIN_SAMPLES is not None and config.MAX_TRAIN_SAMPLES < len(train_dataset):
        ...
        logging.warning(f"OVERRIDE: Limiting training to the first {config.MAX_TRAIN_SAMPLES} samples for development.")
...
def main(args):
    ...
    if args.reward_function == 'f1':
        ...
        logging.info(f"Using ART-E style direct-assessment judge ('{config.JUDGE_MODEL_ID}').")
    ...
    # --- NEW OPTION: Use ToolBrain's native ranking judge ---
        ...
        for item in training_data:
            ...
            item['query'] = item['original_question']
            item['judge_model'] = config.JUDGE_MODEL_ID
        logging.info(f"Using ToolBrain's native ranking-based judge ('{config.JUDGE_MODEL_ID}').")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run training experiments for the ToolBrain Email Agent.")
