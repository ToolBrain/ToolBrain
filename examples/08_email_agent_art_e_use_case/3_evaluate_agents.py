...
def load_test_dataset():
    ...
    """
    logging.info(
        f"Loading TEST questions dataset from '{config.QUESTIONS_DATASET_ID}'..."
    )
    ...
    logging.info(
        f"Using the consistent test split with {len(test_dataset)} samples for evaluation."
    )
...
def classify_answer_with_llm(
    agent_answer: str, gold_answer: str, original_question: str
) -> str:
    """
...
def main(args):
    ...
    print("=" * 50)
    print(
        f"Correctness Rate:      {correctness_rate:.2f}% ({num_correct}/{total_questions} correct)"
    )
    print(f"Efficiency (Avg Turns):  {avg_turns:.2f}")
    print(
        f"Hallucination Rate:    {hallucination_rate:.2f}% ({num_incorrect}/{total_attempted_answers} incorrect attempts)"
    )
    print(
        f"No Answer Rate:        {(num_no_answer / total_questions) * 100:.2f}% ({num_no_answer}/{total_questions} unanswered)"
    )
    print("=" * 50)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate a fine-tuned ToolBrain Email Agent."
    )
