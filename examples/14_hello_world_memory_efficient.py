...
3. Train with explicit, self-documenting parameters in a memory-efficient way (memory vs speed trade-off)
...
brain = Brain(
    ...
    use_grad_accum_train_step=True # uses a memory efficient GRPO optimization where it accumulates gradients for each rollout in num_group_members
)
