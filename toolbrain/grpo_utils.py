...
@dataclass
class Turn:
    """One interaction turn in a trace.
    - prompt_for_model: exact context shown to the LLM at this turn
    - model_completion: raw text generated by the LLM
    - parsed_completion: optional parsed structure (e.g., thought/tool/final)
    - tool_output: environment/tool response observed by the agent
    """
    prompt_for_model: str = ""
    model_completion: str = ""
    parsed_completion: Optional[Dict[str, Any]] = None
    tool_output: str = ""
...
def build_inputs(
    ...
    Args:
        traces: Batch of traces. Each trace is a list of `Turn` dicts. A `Turn` contains:
            - prompt_for_model: context shown to the LLM at this turn
            - model_completion: raw model text (to be optimized)
            - parsed_completion: optional parsed structure (ignored here but kept for consistency)
            - tool_output: environment output (should NOT contribute to loss)
    ...
    for trace, reward in zip(traces, rewards):
        ...
        for turn in trace:
            ...
            model_text = turn.model_completion or ""
            tool_text = turn.tool_output or ""
