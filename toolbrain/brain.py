...
from .core_types import Trace, TraceStep, Example

from typing import Callable, List, Tuple, Optional
import torch
from torch.nn.utils import clip_grad_norm_
from toolbrain.grpo_utils import Policy, copy_model, build_inputs
from toolbrain.losses import grpo_loss
...
if __name__ == "__main__":
    from toolbrain.grpo_utils import Policy, get_llm_and_tokenizer_from_smolagent
    import torch

    llm, tokenzier = get_llm_and_tokenizer_from_smolagent(model_id="gpt2")
    ...
    config = {
        ...
        "epsilon": 0.2,
        "beta": 0.01,
        "mu": 4,                 # Number of policy updates per batch
        "max_grad_norm": 1.0
    }

    algo = GRPOAlgorithm(policy=policy, config=config)

    traces = [ 
        [
            Example(prompt="What is 2 + 2?", completion="4"),
            Example(prompt="What is the capital of France?", completion="Paris"),
        ],
        [
            Example(prompt="What is 2 + 2?", completion="4"),
            Example(prompt="What is the capital of France?", completion="Paris"),
    ...
    rewards = [1.0, 0.8]  # One scalar reward per trace
    updated_policy = algo.train_step(traces, rewards)
    print("Training steps:", algo.training_steps)
